...
window.addEventListener("load", function () {
  ...
  if (typeof supabase === "undefined" || !supabase.createClient) {
    setStatus(
      "Error: no se pudo cargar la librería de Supabase. Verifica tu internet.",
      "err",
    );
    return;
  }
...
function changeMonth(dir) {
  ...
  document.getElementById("detail-body").innerHTML =
    '<div class="no-selection"><div class="icon">🗓️</div><p>Selecciona un día.</p></div>';
...
function renderDetail(dateStr) {
  ...
  document.getElementById("detail-body").innerHTML =
    ...
    '<div class="add-row">' +
    '<input id="new-song" placeholder="Título de la canción" style="flex:1.5" />' +
    ...
    '<div class="add-row">' +
    '<select id="new-minister-select"><option value="">— Seleccionar —</option>' +
    memberOptions +
    "</select>" +
    "<button onclick=\"addMinister('" +
    dateStr +
    "')\">+</button>" +
    "</div>" +
    "</div>" +
    ...
    notes +
    "</textarea>" +
    '<button class="btn-save" onclick="saveNotes(\'' +
    dateStr +
  ...
  el.innerHTML = songs
    .map(function (s, i) {
      ...
      return (
        ...
        "</span>" +
        ytBtn +
        '<button class="item-del" onclick="removeSong(' +
        i +
        ",'" +
        dateStr +
        "')\">×</button></li>"
  ...
  el.innerHTML = mins
    .map(function (m, i) {
      return (
        ...
        (m.instrument || "") +
        '</span><button class="item-del" onclick="removeMinister(' +
        i +
        ",'" +
        dateStr +
        "')\">×</button></li>"
...
function addMinister(dateStr) {
  ...
  for (var j = 0; j < mins.length; j++) {
    if (String(mins[j].id) === String(memberId)) {
      toast("Ya está asignado");
      return;
    }
  }
  mins.push({
  ...
  el.innerHTML = members
    .map(function (m) {
      var initials = m.name
        ...
        .toUpperCase();
      return (
        '<div class="member-chip"><div class="member-avatar">' +
        initials +
        '</div><div class="member-info"><div class="name">' +
        m.name +
        '</div><div class="instr">' +
        (m.instrument || "—") +
        "</div></div></div>"
...
function renderMembersModal() {
  ...
  if (!members.length) {
    el.innerHTML =
      '<div style="color:var(--muted);font-size:0.88rem;margin-bottom:1rem">No hay miembros aún.</div>';
    return;
  }
  el.innerHTML = members
    .map(function (m) {
      return (
        '<div class="modal-member-row"><div><div class="name">' +
        m.name +
        '</div><div class="instr">' +
        (m.instrument || "Sin instrumento") +
        '</div></div><button class="item-del" onclick="deleteMember(' +
        m.id +
        ')">×</button></div>'
      );
    })
